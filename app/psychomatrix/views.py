...
class SquareOfPythagorasViewSet(CreateViewSet):
    queryset = PsychomatrixBaseContent
    serializer_class = DateSerilizer

    def create(self, request, *args, **kwargs):
        data = datetime.strptime(request.data['date'], '%d.%m.%Y')
        if data:
            calculator = Calculator(data)
            numbers = calculator.get_all_numbers()
            basic, addition = get_contents(numbers)
            celebrities = Celebrity.objects.all().values()
            if basic and addition:
                return Response({'mainInfo': [{'basic': basic}, {'addition': addition}, {'celebrities': celebrities}]})
...
#
#     return render(request, "psychomatrix/index.html", context_data)
#
#
...
#     }
#
#     return render(request, "psychomatrix/result.html", context_data)
#
#
